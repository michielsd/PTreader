...
# add stopoverstation, stopovertime
for s in stationtable:
    s[1] = s[1].lower()
    for t in timestable:
        if s[1] == t[1]:
            s.extend([t[2],t[0]]) #time, stopoverstation

#extend stations without match
for s in stationtable:
    if len(s) < 6:
        s.extend(["0","0"])
...
stationcaller = ("""SELECT code FROM stations""")
cur.execute(stationcaller)
stationtuple = cur.fetchall()

addstation = """INSERT INTO stations(name, code, lat, lon, country, stoptime, stopstat)
    VALUES(%s, %s, %s, %s, %s, %s, %s)"""
for row in stationtable:
    if row[1] and row[1] not in stationtuple:
        cur.execute(addstation, tuple(row))
...
#check which travel times are already in database
traveltimecaller = """SELECT departfrom, arriveat FROM traveltimes"""
cur.execute(traveltimecaller)
traveltuple = cur.fetchall()

traininsert = """INSERT INTO trains(train, depstation, arrstation, frequency, deptime, arrtime, difference)
    VALUES(%s, %s, %s, %s, %s, %s, %s)"""
...
for pair in stationpairs:
    if pair in traveltuple:
        print("already committed %s - %s" % (pair[0],pair[1]))
    else:
        listofdep =[]
                ...
                if isinstance(dep, int): #if first line from dict value (= frequency of train)
                    ...
                    for arr in v[lookfrom:]: # CHECKS FOR DIRECT CONNECTIONS
                        if arr[0] == pair[1]: #if combination is made: save dept, arr, difference
                            ...
                            cur.execute(traininsert, (k,pair[0],pair[1],freq,deptime, arrtime,difference))
                ...
                if isinstance(dep, int): #if first line from dict value (= frequency of train)
                    ...
                    for arr in v[lookfrom:]: # CHECKS FOR DIRECT CONNECTIONS
                        if arr[0] == pair[0]: #if combination is made: save dept, arr, difference
                            ...
                            cur.execute(traininsert, (k,pair[1],pair[0],freq,deptime, arrtime,difference))
        ...
        conn.commit()
        print("committed %s - %s" % (pair[0], pair[1]))
